...
using Task3_MissingOpeningInvoices.Helpers;

namespace Task3_MissingOpeningInvoices.ApplicationLogic
{
    public static class ApiInvoicesLogic
    {
        public static ApiInvoiceResponse GetApiInvoices(DateTime dateFrom)
        {
            var apiResponse = new ApiInvoiceResponse();
            try
            {
                var updateRequest = (HttpWebRequest)HttpWebRequest.Create(Config.Get("SdURL") + "GetNewInvoices");
                updateRequest.ContentType = "application/json; charset=utf-8";
                updateRequest.Accept = "application/json";
                updateRequest.Method = "Post";
                updateRequest.Headers.Add(Config.Get("SdUser"), Config.Get("SdPassword"));
                dynamic updateobjectToSend = new ExpandoObject();
                // string CreatedOn = "01/23/2000 1:00 AM";
